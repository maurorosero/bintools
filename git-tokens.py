...
def interactive_prompt(command):
    ...
    # Usuario
    username = Prompt.ask("Usuario o identificador para el servicio")
    # Token (solo para set)
    token = None
    if command == "set":
...
def command_set(args):
    ...
    if not args.service_name or not args.username:
        service_name, username, token = interactive_prompt("set")
    else:
        service_name, username, token = args.service_name, args.username, args.token
    service, mode, usage = parse_service_name(service_name)
    if token is None:
        token = prompt_token()
    token_enc = encrypt_token(token, method)
...
def command_get(args):
    ...
    if not args.service_name or not args.username:
        ...
        service_name, username = args.service_name, args.username
    service, mode, usage = parse_service_name(service_name)
    token_enc = keyring.get_password(build_service_name(service, mode, usage, method), username)
    if token_enc:
        token = decrypt_token(token_enc, method)
...
def command_list_services(args=None):
    ...
    print("  git-tokens.py set --b64 github-personal usuario")
    print("  git-tokens.py get --b64 github-personal usuario")

def main():
    version, author = get_header_metadata()
    epilog = "\n\nEjemplo de uso con base64 (por defecto):\n  git-tokens.py set --b64 github-personal usuario\n  git-tokens.py get --b64 github-personal usuario"
    ...
    set_pos_group.add_argument("username", nargs="?", help="Usuario o identificador para el servicio")
    ...
    get_pos_group.add_argument("username", nargs="?", help="Usuario o identificador para el servicio")
    ...
    del_pos_group.add_argument("username", nargs="?", help="Usuario o identificador para el servicio")
