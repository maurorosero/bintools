...
# -*- coding: utf-8 -*-
"""
Script CLI para gestionar tokens de autenticación de servicios de AI
...
Last modified: 2025-06-08 23:11:33
...
AI_SERVICES = ["ai-openai", "ai-anthropic", "ai-huggingface", "ai-google", "ai-azure", "ai-deepseek"]
SERVICE_LABELS = {
    "ai-openai": "OpenAI (GPT-4)",
    "ai-anthropic": "Anthropic (Claude)",
    "ai-huggingface": "Hugging Face",
    "ai-google": "Google AI (Gemini)",
...
def interactive_prompt(command):
    ...
    try:
        console = Console()
        # Elegir servicio
        table = Table(title="Servicios AI soportados", show_lines=True)
        ...
        for idx, svc in enumerate(AI_SERVICES, 1):
            ejemplo = f"{svc}-personal"
            table.add_row(str(idx), SERVICE_LABELS[svc], ejemplo)
        console.print(table)
        svc_idx = IntPrompt.ask("Selecciona el servicio (número)", choices=[str(i) for i in range(1, len(AI_SERVICES)+1)])
        service = AI_SERVICES[int(svc_idx)-1]
        # Uso
...
def parse_service_name(service_name):
    """Parsea y valida el nombre del servicio.

    Args:
        service_name (str): Nombre del servicio en formato 'ai-[service]-[uso]'

    Returns:
        tuple: (service, usage)

    Raises:
        SystemExit: Si el formato no es válido o el servicio no está soportado
    """
    parts = service_name.split("-")
    if len(parts) != 3:
        print("Error: El formato del nombre de servicio debe ser 'ai-[service]-[uso]'.")
        print("Ejemplo: ai-openai-docs, ai-anthropic-work")
        sys.exit(1)

    prefix, service, usage = parts
    if prefix != "ai":
        print("Error: El nombre del servicio debe comenzar con 'ai-'.")
        print("Ejemplo: ai-openai-docs, ai-anthropic-work")
        sys.exit(1)

    full_service = f"ai-{service}"
    if full_service not in AI_SERVICES:
        print(f"Error: Servicio '{full_service}' no soportado. Usa 'list-services' para ver los válidos.")
        sys.exit(1)

    return full_service, usage

def build_service_name(service, usage, encrypt_method):
    if encrypt_method == "b64":
        return f"{service}-{usage}"
    else:
        return f"{service}-{usage}-{encrypt_method}"
...
def set_token(service_name, username, token=None, method="b64"):
    service, usage = parse_service_name(service_name)
    if token is None:
        token = prompt_token()
    token_enc = encrypt_token(token, method)
    keyring.set_password(build_service_name(service, usage, method), username, token_enc)
    print(f"✓ Token guardado para {SERVICE_LABELS[service]} ({usage}) [{username}] (método: {method})")

def get_token(service_name, username, method="b64"):
    service, usage = parse_service_name(service_name)
    token_enc = keyring.get_password(build_service_name(service, usage, method), username)
    if token_enc:
        token = decrypt_token(token_enc, method)
        print(f"Token para {SERVICE_LABELS[service]} ({usage}) [{username}]:\n{token}")
    else:
        print(f"No se encontró token para {SERVICE_LABELS[service]} ({usage}) [{username}]")

def delete_token(service_name, username):
    service, usage = parse_service_name(service_name)
    try:
        keyring.delete_password(build_service_name(service, usage, "b64"), username)
        print(f"✓ Token eliminado para {SERVICE_LABELS[service]} ({usage}) [{username}]")
    except keyring.errors.PasswordDeleteError:
        print(f"No se encontró token para {SERVICE_LABELS[service]} ({usage}) [{username}]")

def list_services():
    print("Servicios AI soportados:")
    print("  ai-openai: OpenAI (GPT-4)")
    print("  ai-anthropic: Anthropic (Claude 2)")
    print("  ai-huggingface: Hugging Face")
    print("  ai-google: Google AI (Gemini Pro)")
    print("  ai-azure: Azure OpenAI")
    print("  ai-deepseek: DeepSeek AI")
    print("\nEl campo 'uso' es un identificador libre, por ejemplo: personal, work, empresaX, etc.")
    print("Ejemplo de nombre de servicio: ai-openai-personal")
    print("\nEjemplo de uso:")
    print("  ai-tokens.py set ai-openai-personal              # Usuario del SO automático")
    print("  ai-tokens.py set ai-openai-personal otrouser     # Usuario específico")
    print("  ai-tokens.py get ai-openai-personal              # Usuario del SO automático")
    print("  ai-tokens.py get ai-openai-personal otrouser     # Usuario específico")
...
def command_delete(args):
    ...
    service, usage = parse_service_name(service_name)
    try:
        keyring.delete_password(build_service_name(service, usage, method), username)
        print(f"✓ Token eliminado para {SERVICE_LABELS[service]} ({usage}) [{username}] (método: {method})")
    except keyring.errors.PasswordDeleteError:
        print(f"No se encontró token para {SERVICE_LABELS[service]} ({usage}) [{username}]")
...
def main():
    ...
    try:
        ...
        epilog = "\n\nEjemplo de uso con base64 (por defecto):\n  ai-tokens.py set ai-openai-personal              # Usuario del SO automático\n  ai-tokens.py set ai-openai-personal otrouser     # Usuario específico\n  ai-tokens.py get ai-openai-personal              # Usuario del SO automático"
        parser = argparse.ArgumentParser(
            description="Gestor de tokens de autenticación para servicios de AI usando keyring.",
            add_help=False,
            epilog=epilog
        )
        parser.add_argument(
        ...
        set_pos_group.add_argument("service_name", nargs="?", help="Nombre de servicio en formato '[service]-[uso]'")
        ...
        get_pos_group.add_argument("service_name", nargs="?", help="Nombre de servicio en formato '[service]-[uso]'")
        ...
        del_pos_group.add_argument("service_name", nargs="?", help="Nombre de servicio en formato '[service]-[uso]'")
